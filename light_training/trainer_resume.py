...
class Trainer:
    ...
    def train(self,
        ...
        for epoch in range(0, self.max_epochs):
            ...
            val_outputs = []
            if ((self.epoch + 1) % self.val_every == 0 or self.epoch == 0)\
        ...
        if self.local_rank == 0:
            with tqdm(total=len(loader)) as t:
                for idx, batch in enumerate(loader):
                    ...
                    if self.auto_optim and loss is not None:
                        loss.backward()
                        self.optimizer.step()
    ...
    def training_step2(self, batch):
        raise NotImplementedError
    def training_step3(self, batch):
        raise NotImplementedError
    ...
    def load_state_dict2(self, weight_path, strict=True):
        sd = torch.load(weight_path, map_location="cpu")
