...
def sliding_window(
    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        seg_prob_out = predictor(window_data, task_id,pred_type="denoise")
        N_b, N_q, N_c, N_d, N_h, N_w = seg_prob_out.shape
        preds_convert = torch.zeros(size=(N_b, N_c, N_d, N_h, N_w)).cuda()
        for i_b in range(N_b):
            preds_convert[i_b] = seg_prob_out[i_b, int(task_id)]
        seg_prob_out = preds_convert
...
def sliding_window4(
    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        seg_prob_out = predictor(window_data, task_id,pred_type="denoise")
        N_b, N_q, N_c, N_d, N_h, N_w = seg_prob_out.shape
        preds_convert = seg_prob_out.view(N_b, N_q, N_d, N_h, N_w )
        seg_prob_out = preds_convert
        seg_prob_tuple: Tuple[torch.Tensor, ...]
        if isinstance(seg_prob_out, torch.Tensor):
            seg_prob_tuple = (seg_prob_out,)
